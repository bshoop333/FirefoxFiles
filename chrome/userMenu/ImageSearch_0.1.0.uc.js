...
// @note			[20160621]修复百度post搜图会打开两个标签的问题。
// ==/UserScript==
...
location == 'chrome://browser/content/browser.xul' && (function () {
	var imageSearch = {
		...
		baiduApiKey: ''8ed34d781943b5f2af11497ee5ef1fea, //在这里输入你的百度apikey, key获取地址http://apistore.baidu.com/apiworks/servicedetail/146.html

		bgImage: true, //是否对背景图起效

		get contextMenu() {return document.getElementById('contentAreaContextMenu')},
		...
		site: {
			...
			'Yandex': 		{//网站使用json查询，如果要使用本地的图片只能在网站上上传了。
				icon: true,
				both:{url:'http://yandex.com/images/search', method: 'GET',
					parameters:{
			...
			'E-Hentai': 	{//本人无法打开这个网站
				icon: true,
				both:{url:'http://ul.e-hentai.org/image_lookup.php', method: 'POST',
					parameters:{
		...
		methodPostURL: function(siteURL, siteVars) {
			...
			gBrowser.loadURIWithFlags(siteURL, Ci.nsIWebNavigation.LOAD_FLAGS_NONE, Services.io.newURI(siteURL, null, null), null, postData);
		},
